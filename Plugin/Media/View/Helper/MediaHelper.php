...
class MediaHelper extends AppHelper{
	...
	public function textarea($field, $editor = 'ckeditor', $options = array()) {
        ...
        if (isset($this->request->data[$this->ref]['id']) && !$this->explorer) {
			$html .= '<input type="hidden" id="explorer" value="' . $this->Html->url('/media/medias/index/'.$this->ref.'/'.$this->request->data[$this->ref]['id']) . '">';
			$this->explorer = true;
    	}
